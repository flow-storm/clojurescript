     ...
     (core/letfn [(get-delegate []
       ...
       `(do
          (set! (. ~sym ~(get-delegate-prop))
                ~(storm-utils/merge-meta `(~(with-meta `fn (meta sym)) (~(vec sig) ~@body))
  ...
  (core/letfn [(dest-args [c]
               ...
               (fn-method [name [sig & body :as method]]
                 (if (some '#{&} sig)
                   ...
                   (let [fn-form (storm-utils/merge-meta `(~(with-meta `fn (core/meta name)) ~method) {:cljs.storm/fn-trace-name (core/str name)})]
                     `(set!
