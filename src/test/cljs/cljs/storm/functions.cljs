(ns cljs.storm.functions
  (:require [clojure.test :refer [deftest is use-fixtures]]
            [cljs.storm.tests.functions :as fns]
...
(deftest empty-body-fn-test
  (let [r (fns/empty-body 40 2)]
    ...
    (is (= [[:fn-call "cljs.storm.tests.functions" "empty-body" [40 2] 110441699]
            [:bind "a" 40 ""]
            [:bind "b" 2 ""]
...
(deftest simple-fn-test
  (let [r (fns/simple 40 2)]
    ...
    (is (= [[:fn-call "cljs.storm.tests.functions" "simple" [40 2] 1954697464]
            [:bind "a" 40 ""]
            [:bind "b" 2 ""]
...
(deftest defed-fn-test
  (let [r (fns/defed)]
    (is (= 42 r) "function return should be right.")
    (is (= [[:fn-call "cljs.storm.tests.functions" "defed" [] -699754393]
...
(deftest multi-arity-fn-test
  (let [r (fns/multi-arity 40)]
    (is (= 42 r) "function return should be right.")
    (is (= [[:fn-call "cljs.storm.tests.functions" "multi-arity" [40] -876490731]
            [:bind "a" 40 ""]
            [:expr-exec 40 "2,1,1" -876490731]
            [:fn-call "cljs.storm.tests.functions" "multi-arity" [40 2] -876490731]
            [:bind "a" 40 ""]
            [:bind "b" 2 ""]
...
(deftest args-destructuring-fn-test
  (let [r (fns/args-destructuring {:a 40 :b 2})]
    (is (= 42 r) "function return should be right.")
    (is (= [[:fn-call "cljs.storm.tests.functions" "args-destructuring" [{:a 40, :b 2}] 713448837]
            [:bind "a" 40 ""]
            [:bind "b" 2 ""]
...
(deftest variadic-fn-test
  (let [r (fns/variadic 1 2 3 4)]
    ...
    (is (= '[[:fn-call "cljs.storm.tests.functions" "variadic" [(1 2 3 4)] 1269596501]
             [:bind "nums" (1 2 3 4) ""]
...
(deftest tail-recursive-fn-test
  (let [r (fns/tail-recursive 0 2)]
    ...
    (is (= [[:fn-call "cljs.storm.tests.functions" "tail-recursive" [0 2] 1815279187]
            [:bind "s" 0 ""]
            [:bind "n" 2 ""]
...
(deftest recursive-fn-test
  (let [r (fns/factorial 4)]
    ...
    (is (=  [[:fn-call "cljs.storm.tests.functions" "factorial" [4] 71712880]
             [:bind "n" 4 ""]
             [:expr-exec 4 "3,1,1" 71712880]
             [:expr-exec false "3,1" 71712880]
             [:expr-exec 4 "3,3,1" 71712880]
             [:expr-exec "#object[...]" "3,3,2,0" 71712880]
             [:expr-exec 4 "3,3,2,1,1" 71712880]
             [:expr-exec 3 "3,3,2,1" 71712880]
             [:fn-call "cljs.storm.tests.functions" "factorial" [3] 71712880]
             [:bind "n" 3 ""]
             ...
             [:fn-call "cljs.storm.tests.functions" "factorial" [2] 71712880]
             [:bind "n" 2 ""]
             ...
             [:fn-call "cljs.storm.tests.functions" "factorial" [1] 71712880]
             [:bind "n" 1 ""]
             ...
             [:fn-call "cljs.storm.tests.functions" "factorial" [0] 71712880]
             [:bind "n" 0 ""]
...
(deftest multimethods-test
  (let [r (fns/area {:type :square :side 4})]
    ...
    (is (=  [[:fn-call "cljs.storm.tests.functions" "area" [{:type :square, :side 4}] -1257757404]
             [:bind "side" 4 ""]
